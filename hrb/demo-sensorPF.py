...
from numpy import ones,asarray,linspace,arange,argmax,percentile
...
def _animation(fig):
  ...
  wgt = ones(Num)
  # Line
  abLine = asarray([-.5-0.4j,.8+0.3j])
  # Actual position
  pos = 0.5+0.5j
  while True:
    ### "robot" simulation
    # Move "robot"
    step = (randn() + randn()*1j) * stepSize-pos*0.05 +sin(time()/2)*0.06
    pos += step
    ...
    idx = (wgt<percentile(wgt,minP)).nonzero()[0]
    if any(idx):
      pts[idx] = pts[best]
      wgt[idx] = 1.0
    fig.clf()
