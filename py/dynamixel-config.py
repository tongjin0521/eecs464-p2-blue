...
class dynamixelConfigurator:
    ...
    def parseArgs( self, argv ):
        ...
        if len(argv) < 1:
            self.usage()
            sys_exit('No arguments passed, see Usage \n')
    ...
    def scanAll( self ):
        ...
        """
        nid_str = []
        for baud in self.p.bus.scanBauds(useFirst=False):
    ...
    def write_config( self, nid ):
        ...
        for name, val in self.cfg.iteritems():
            ...
            if name == 'ID':
                sys_exit(" Setting ID through .yml file not allowed, see --help")
        ...
        # Write baudrate last 
        node.mem_write_sync( getattr( node.mm, name ), baud )
