...
from .pololu import (
  ...
  ServoModule as pololu_ServoModule,
  CR, UB
)
...
class Protocol( pololu_Protocol ):
    def __init__(self, *arg, **kw):
      ...
      self.count=6 # Current code supports 6 motors on a polowixel
      self.DEBUG=DEBUG

    def update(self, t):
      # Superclass update
      pololu_Protocol.update(self,t)
      # Bus update
      self.bus.update(t)
      ...
      try:
        ...
        return
      for nid,ii in self.nodes.items():
        val = msg[ii]
        if nid in self.pnas:
    ...
    def generatePNA(self, nid):
      ...
      """
      if nid & CR:
        pna = ProtocolNodeAdaptor_CR(self, nid)
      elif nid & UB:
        pna = ProtocolNodeAdaptor_UB(self, nid)
      else:
        raise TypeError("Node ID 0x%02x does not match any known module type" % nid)
      self.pnas[nid] = pna
      return pna

class ProtocolNodeAdaptor_UB( pololu_ProtocolNodeAdaptor ):
    """
    ...
    def __init__(self,*arg,**kw):
      pololu_ProtocolNodeAdaptor.__init__(self,*arg,**kw)
      self.pos = 0
...
class ProtocolNodeAdaptor_CR( AbstractNodeAdaptor ):
    """
    Utilizes the protocol along with nid to create an interface for a
    specific module
    """
    ...
    # Uncalibrated lower bound for position measurement
    DEFAULT_LB = 300

    # Uncalibrated upper bound for position measurement
    DEFAULT_UB = 2700

    # Maximal RPM supported by servo
    MAXRPM = 140
...
class ServoModuleUB( pololu_ServoModule ):
    RAW_POS_OFS = 0
    RAW_POS_SCALE = 1.

    def __init__(self, node_id, typecode, pna, *argv, **kwarg):
      pololu_ServoModule.__init__(self, node_id, typecode, pna, *argv, **kwarg)

    def get_pos(self):
      return (self.pna.getRawPos()-self.RAW_POS_OFS)*self.RAW_POS_SCALE

    def isCalibrated(self):
      return True

class ServoModuleCR( ServoModule ):
    """
    ...
    def set_rpm(self, target):
      ...
      if self.mode == 1:
        ...
        return self.pna.set_rpm(target)
    ...
    def get_pos(self):
      ...
      NOTE: before the module has made a complete rotation, this result is highly suspect...
      """
      if self.pos is None:
        return None
      return int(self.pos*self.SCL-self.OFS)
