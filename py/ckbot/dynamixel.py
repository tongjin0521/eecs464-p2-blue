...
class DX_MX_Module(DynamixelModule):
    ...
    #
    #     INPUT:
    #       val -- units in 1/100s of degrees. Must be within 9000 of last input/current positon
    #     """
    #
    #     if self.mode != 2:
    ...
    #         return
    #
    #     if abs(val-self.last_pos) > 9000 :
    #         raise ValueError ("Value must be < %s or > %s" % (self.last_pos+9000,self.last_pos-9000))
    #         return
    #
    #     if self.count > 216000 or self.count < 0 :
    #         self.pna.mem_write_fast(self.mcu.multi_turn_offset,12285)
    #         self.count = self.get_pos()
    #
    #     self.count += (val-self.last_pos)
    #     self.last_pos = val
    #
    #     return self.pna.mem_write_fast(self.mcu.goal_position,
    #       self.ang2dynamixel(self.count))
