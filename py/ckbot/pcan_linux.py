...
class CANIX(AbstractCANIX):
      def __init__(self):
            O_RDWR = 2
            device_path = "/dev/pcan32"
            self.h = LICAN.open(device_path,O_RDWR)
            if self.h == 0:
                  raise CANIOError(
'''Failed to open device: %s. Can handle is zero.
   Is the pcan module loaded (use 'sudo modprobe pcan')?
   Is the driver compiled (use 'ckbot/build-pcan-module.sh')?
'''%device_path)
            
      def init(self,baudrate_code = 0x11C, msgtype = MSGTYPE_STANDARD, dev = "/dev/pcan32"):
