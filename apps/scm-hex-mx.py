...
class SCMHexApp( JoyApp ):
  ...
  def _fcp_fun( self, phase ):
      ...
      aL = (phase-0.5)*36000
      aR = (((phase+0.5) % 1.0)-0.5)*36000
      aDes = [aL,aL,aL,aR,aR,aR]
      for leg, des in zip( self.triL + self.triR, aDes ):
          # Set to RPM value
          leg.set_rpm(15)
          # Measure velocity and position error
          ang = leg.get_ang()
          if abs(ang)<10000: # if a measurement in range --> store error
              p.append( ang - aDes )
          else: # else ignore the error 
              p.append( nan )
          if not isnan(leg._v):
              v.append(leg._v)
              vl.append(leg)
      # Compute phase for left and right tripods
      pL = phase
      pR = (phase+0.5) % 1.0
      # Compute desired angles
      des = [ (pL-0.5)*36000, (pr-0.5)*36000 ] * 3
      
      # If we have velocity measurements, adjust gains in each leg
      #   to pull speeds closer together
      if v:
          if "v" in DEBUG:
              progress( "velocity " + str(asfarray(v).round(2)))
          mv = prod(v) ** (1.0/len(v))
          for l,lv in zip(vl,v):
              progress( "adj(%s) by %.2f" % (l.servo.name,lv/mv))
              leg.adjustV( lv / mv )
